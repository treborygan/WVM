DROP TRIGGER IF EXISTS special_visual_versions_require_validation;
...
WHEN NEW.published_at IS NOT NULL
    ...
    AND (
        ...
        OR COALESCE(json_type(NEW.special_validation_json, '$.state') = 'text', 0) = 0
        OR COALESCE(json_extract(NEW.special_validation_json, '$.state') = 'validated', 0) = 0
        OR COALESCE(json_type(NEW.special_validation_json, '$.reviewer_id') = 'text', 0) = 0
        OR COALESCE(length(trim(json_extract(NEW.special_validation_json, '$.reviewer_id'))), 0) = 0
        OR COALESCE(json_type(NEW.special_validation_json, '$.evidence_ref') = 'text', 0) = 0
        OR COALESCE(length(trim(json_extract(NEW.special_validation_json, '$.evidence_ref'))), 0) = 0
        OR COALESCE(json_type(NEW.special_validation_json, '$.reviewed_at') = 'text', 0) = 0
        ...
        OR COALESCE(date(substr(json_extract(NEW.special_validation_json, '$.reviewed_at'), 1, 10))
            = substr(json_extract(NEW.special_validation_json, '$.reviewed_at'), 1, 10), 0) = 0
        OR COALESCE(substr(json_extract(NEW.special_validation_json, '$.reviewed_at'), 12, 2) BETWEEN '00' AND '23', 0) = 0
        OR COALESCE(substr(json_extract(NEW.special_validation_json, '$.reviewed_at'), 15, 2) BETWEEN '00' AND '59', 0) = 0
        OR COALESCE(substr(json_extract(NEW.special_validation_json, '$.reviewed_at'), 18, 2) BETWEEN '00' AND '59', 0) = 0
    )
BEGIN
